...
void output_cur(int n)
{
	...
	int d=used[n-1];
	NESET::final_edge_set r = nes.normalize(n);
#if 1
	NESET nes2(line_count-d,r.edges);
	NESET::final_edge_set r2= nes2.normalize(n);
	char outputbuf2[4*MAXLINE_COUNT];
	r2.show(outputbuf2);
	if(stdformat!=outputbuf2)return;
#endif
...
void process_one_line(char *input)
{
    ...
    NESET::final_edge_set r = neset.normalize(TARGET_NODES-1);
    r.show(outputbuf);
    stdformat=outputbuf;
    tmpfile2 = fopen("tmpfilef","w");
    if(tmpfile2==NULL){
...
#endif
    tsearch(TARGET_NODES-1);
    fclose(tmpfile2);
    tmpfile2=NULL;
    system("sort -u tmpfilef |/home/zdu/solve8s");
}
