...
#endif
#define NUMP 4
int target[NUMP]={6,6,3,2};//target product 2^6*3^6*5^3*7^2
int list[TNUM][NUMP];
long llist[TNUM];
time_t starttime;
void initlist()
{
	...
	for(i=0;i<TNUM;i++){
		long v=1L;
		for(j=0;j<list[i][0];j++)v*=2;
		for(j=0;j<list[i][1];j++)v*=3;
		for(j=0;j<list[i][2];j++)v*=5;
		for(j=0;j<list[i][3];j++)v*=7;
		llist[i]=v;
	}
	for(i=0;i<TNUM;i++){
		printf("%02d:{",i);
