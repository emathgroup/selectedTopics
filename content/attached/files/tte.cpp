...
int getStringResult(const set<string>& allwords, const string& s, int& the_total)
{
	Part parts;
	int ttcount=0;
	int tsum=0;
...
int main(int argc, char *argv[])
{
	...
	string bests;
	set<string>::iterator sit;
	for(sit=allwords.begin();sit!=allwords.end();++sit){
		the_try=getStringResult(allwords,*sit,total);
		if(the_try<best_try||the_try==best_try&&total<best_total){
			cout<<"Find "<<*sit<<"\n";
			fflush(stdout);
			best_try=the_try;
			best_total=total;
	...
	dump_result(allwords,7,bests, 0);
