...
void init_for_N()
{
    ...
    for(i=0;i<cands.size();i++)for(j=i+1;j<cands.size();j++){
	 IS is;
	 is.cnpair=1;
	 is.pairs[0]=cands[i];//create pseudo edge link cands[i] & cands[j]
	 is.pairs[1]=cands[j];
	 is.mask=0L;
...
bool makenewis(IS& nis, const IS& ois, int c1, int c2, int step)//connect c1 & c2 into one group
{
	...
	if((index1^1)==index2){//merge them together
	    ...
	    __int128_t val=(ONE<<step)-2;
	    if(((ois.mask|(ONE<<c1)|(ONE<<c2))&val)!=val)return false;
	    nis.mask = val;
	    nis.cnpair=0;
	    return true;
	}
...
void update_for_x(int x)
{
	...
	if(mit->first.mask &(ONE<<x)){//The number has been masked, copy it to next level and do nothing
	    IS nis=mit->first; nis.mask&=~(ONE<<x);//remove x from mask, it is not required any more
	...
	if(xindex<0){//number x has never been used
		...
		if(mit->first.mask&(ONE<<cands[i]))continue;//The number has been fully consumed
		for(j=i+1;j<cands.size();j++){
		    if(mit->first.mask&(ONE<<cands[j]))continue;
		    IS nis;
		...
		if(mit->first.mask&(ONE<<cands[i]))continue;
		IS nis;
...
#pragma omp atomic update
    ...
    int xindex = is.get_x_index(x);
        if(xindex<0){//number x has never been used
            for(i=0;i<cands.size();i++){//try to connect it to two cands
                if(is.mask&(ONE<<cands[i]))continue;//The number has been fully consumed
                for(j=i+1;j<cands.size();j++){
                    if(is.mask&(ONE<<cands[j]))continue;
                    IS nis;
            ...
            for(i=0;i<cands.size();i++){
                if(is.mask&(ONE<<cands[i]))continue;
                IS nis;
