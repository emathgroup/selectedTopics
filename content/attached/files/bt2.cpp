...
void dump_result()
{
	...
	if(dispcount<0||dispcount>vd.size())dispcount=vd.size();
	...
	for(i=firstrow;i<lastrow+1;i++)for(j=firstcol;j<lastcol+1;j++){
		if(useinfo[i][j]>=0){
			...
			if(k>=dispcount)
			   ve.push_back(useinfo[i][j]);
		}
	}
	sort(ve.begin(),ve.end());
	for(i=0;i<ve.size();++i){
		printf("%s",cis[ve[i]].s.c_str());
	}
	printf("\n\n");
	...
	do{
		...
		do{
			...
			for(kk=0;kk<2;kk++){
				...
				if(rowfirst[kk]&&ui.word_off==3&&lc==2&&rc>=1&&col>=3&&idms[ui.word_id].c[0]==useinfo[row][col-3] ||
						rowfirst[kk]&&ui.word_off==0&&rc==2&&lc>=1&&col<=2*RANGE-3&&idms[ui.word_id].c[3]==useinfo[row][col+3]){
					int k;
					int startoff=0;
					if(ui.word_off==3)startoff=-3;
					for(k=1;k<=2;k++){
						useinfo[row][col+startoff+k]=idms[ui.word_id].c[k];
						Loc loc;loc.row=row;loc.col=col+startoff+k;
						locstack.push_back(loc);
					}
					wordadded++;
					idms[ui.word_id].used=1;
				...
				if(!rowfirst[kk]&&ui.word_off==3&&uc==2&&dc>=1&&row>=3&&idms[ui.word_id].c[0]==useinfo[row-3][col] ||
						!rowfirst[kk]&&ui.word_off==0&&dc==2&&uc>=1&&row<=2*RANGE-3&&idms[ui.word_id].c[3]==useinfo[row+3][col]){
					int k;
					int startoff=0;
					if(ui.word_off==3)startoff=-3;
					for(k=1;k<=2;k++){
						useinfo[row+startoff+k][col]=idms[ui.word_id].c[k];
						Loc loc;loc.row=row+startoff+k;loc.col=col;
						locstack.push_back(loc);
					}
					wordadded++;
					idms[ui.word_id].used=1;
