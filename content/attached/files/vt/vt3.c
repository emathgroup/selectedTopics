...
int main()
{
    ...
    for(a1=1;a1<MAXN;a1++)for(a2=a1+1;a2<MAXN;a2++)for(a3=a2+1;a3<MAXN;a3++)
	    for(a4=a3+1;a4<MAXN;a4++)for(a5=a4+1;a5<MAXN;a5++){
		   int r=a1*a1+a2*a2+a3*a3+a4*a4+a5*a5;
		   double h=sqrt(r); int g=(int)round(h);
		   if(fabs(h-round(h))<1E-8&&r==g*g){
			   ss=g;s[0]=a1;s[1]=a2;s[2]=a3;s[3]=a4;s[4]=a5;
			   printf("Search: %d^2+%d^2+%d^2+%d^2+%d^2=%d^2\n",a1,a2,a3,a4,a5,g);
    inita(a,U+1,ss);
    do{
	if(a[0]>s[0])break;
	inita(b,V+1,ss);
