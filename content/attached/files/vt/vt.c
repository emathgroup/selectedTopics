...
char fill[MAXF];
int searchfill(int edge, int sti, int edi, int left_count)
{
    int i,j,k,s,t;
    if(left_count==0){//search succ;
	    return 1;
    }
    for(i=0;i<edge;i++)for(j=0;j<edge;j++){
	if(bb[i][j]==0){
	    for(k=sti;k<edi;k++){
		if(fill[k])continue;
		if(i+tdata[idata[k]].x<=edge&&j+tdata[idata[k]].y<=edge){
		    int pass=1;
		    for(s=0;s<tdata[idata[k]].x;s++)for(t=0;t<tdata[idata[k]].y;t++){
			    if(bb[i+s][j+t]!=0){pass=0;goto exitloop1;}
		    }
exitloop1:
		    if(pass){
			for(s=0;s<tdata[idata[k]].x;s++)for(t=0;t<tdata[idata[k]].y;t++)bb[i+s][j+t]=k-sti+1;
			fill[k]=1;
			if(searchfill(edge, sti, edi, left_count-1))return 1;
			fill[k]=0;
			for(s=0;s<tdata[idata[k]].x;s++)for(t=0;t<tdata[idata[k]].y;t++)bb[i+s][j+t]=0;
		    }
		}
		if(i+tdata[idata[k]].y<=edge&&j+tdata[idata[k]].x<=edge){
		    int pass=1;
		    for(s=0;s<tdata[idata[k]].x;s++)for(t=0;t<tdata[idata[k]].y;t++){
...
exitloop2:
		    if(pass){
			for(s=0;s<tdata[idata[k]].x;s++)for(t=0;t<tdata[idata[k]].y;t++)bb[i+t][j+s]=k-sti+1;
			fill[k]=1;
			if(searchfill(edge, sti, edi, left_count-1))return 1;
			fill[k]=0;
			for(s=0;s<tdata[idata[k]].x;s++)for(t=0;t<tdata[idata[k]].y;t++)bb[i+t][j+s]=0;
		    }
		}
	    }
	}
