...
int a[U+1]={1,2,2,3,5},b[V+1]={1,2,3,7};
rec_t tdata[(U+1)*(V+1)];
int used[(U+1)*(V+1)];
int idata[(U+1)*(V+1)];
int num[7];
...
int main()
{
    ...
    for(a1=1;a1<MAXN;a1++)for(a2=a1+1;a2<MAXN;a2++)for(a3=a2+1;a3<MAXN;a3++)
	    for(a4=a3+1;a4<MAXN;a4++)for(a5=a4+1;a5<MAXN;a5++)for(a6=a5+1;a6<MAXN;a6++){
		   int r=a1*a1+a2*a2+a3*a3+a4*a4+a5*a5+a6*a6;
		   double h=sqrt(r); int g=(int)round(h);
		   if(fabs(h-round(h))<1E-8&&r==g*g){
			   ss=g;s[0]=a1;s[1]=a2;s[2]=a3;s[3]=a4;s[4]=a5;s[5]=a6;
			   printf("Search: %d^2+%d^2+%d^2+%d^2+%d^2+%d^2=%d^2\n",a1,a2,a3,a4,a5,a6,g);
    inita(a,U+1,ss);
    do{
	if(a[0]>s[0])break;
	inita(b,V+1,ss);
