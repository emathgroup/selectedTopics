...
# 摘要
mathe[于2008年4月](https://bbs.emath.ac.cn/thread-331-1-1.html) 引用[百度贴吧中东方角落的一个问题](http://tieba.baidu.com/f?kz=42429501)  
...
[首先](https://bbs.emath.ac.cn/thread-332-1-1.html) 对于多项式$h(x)=x^{m+n}-2x^n+1$,有$h'(x)=x^{n-1}((m+n)x^m-2n)$, 所以$h'(1)\lt 0$而且在(0,1)区间均有$h'(x)\lt 0$。再由$h(0)=1,h(1)=0,h(\sqrt[m]{2})\gt 0$,我们容易得出存在唯一的$1\lt x_{m,n}\lt \sqrt[m]{2}$使得$h(x_{m,n})=0$并且对于任意$1\lt r\lt x_{m,n}, 必然h(r)\lt 0$ 。  
选择$f(x)=x^m-2, g(x)=x^{-n}$, 并且选择复平面上一个圆环 $r\lt |z| \lt R$ (其中 $1\lt r\lt x_{m,n}, R\ge 4$ ), 那么可以得出在内边界上$|f(z)|= 2-r^m \gt r^{-n}=|g(z)|$；在外边界上$|f(z)| \ge |z|-2\ge 2 \gt |z|^{-n}$,  
所以根据儒歇定理，函数$f(x)$和$f(x)+g(x)$在圆环$r\lt |x|\lt R$内零点数目相同。  
...
我们可以先处理x为有理数情况，然后再选择两个非常接近$\pi$的有理数a和b,$a\gt \pi\gt b$,分别计算$p(a)$和$p(b)$,那么必然有$p(a)\lt p(\pi)\lt p(b)$。
在a,b都很接近$\pi$时，这两个结果应该可以很好的逼进$p(\pi)$。  
所以[问题转为](https://bbs.emath.ac.cn/forum.php?mod=redirect&goto=findpost&ptid=331&pid=2852&fromuid=20) 对于有理数$\frac nm$,计算$p(\frac nm)$，这个问题可以转化为A赢了前进m米，B赢了前进n米的问题($n\gt m,(m,n)=1$)。  
...
在得出$q(k)=a_1 x_1^k+a_2 x_2^k +... +a_m x_m^k$后，其中$a_1,a_2,...,a_m$， 可以利用$q(0)=1,q(-1)=1,...,q(-m+1)=1$使用待定系数法解出。
在这个计算过程中，我们记$y_i=\frac 1{x_i}$,所以$|y_i|\gt 1$, 
...
我们可以计算出$q(n)=a_1x_1^n+a_2x_2^n+...+a_mx_m^n$
而最终所求的概率为$\frac 1 2+\frac{q(n)}2$  
这个算法时间复杂度最大为$O(m^3)$（解m阶线性方程组的时间）  
...
## 图解分析
zgg\_\_根据mathe前面给出的算法，进一步给出了$p(\frac mn)$更多的结果，并给出了[如下的图片](https://bbs.emath.ac.cn/forum.php?mod=redirect&goto=findpost&ptid=331&pid=3644&fromuid=20) 结果  
![rw](../images/rw.gif)  
其中横坐标为有理数$\frac mn$,纵坐标为函数$p$  
可以看出函数图像断断续续。zgg\_\_判断应该所有的有理数点都是间断点，mathe也觉得从概率模型可以得出所有有理点是间断点而所有无理数点连续，这真是一个很有意思的函数。  
...
$\frac12+\sum_{n=1}^{\infty}\frac{A_n}{2^{b_n}}$  
其中  
$b_n=\lceil n\pi\rceil+n$  
$A_n = C_{b_{n-1}-1}^{n-1}-\sum_{k=1}^{n-2}A_kC_{b_{n-k-1}-1}^{n-k}$  
$A_1=1, A_2=4,A_3=22, A_4=140, A_5=969,A_6=7084,\dots$
