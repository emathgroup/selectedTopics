...
[n是奇数](../attached/scale/scale_odd.txt)和[n是偶数](../attached/scale/scale_even.txt)情况的解， 但是为了能够更加有效的搜索，对数据做了限制，要求最小的数字等于0（这个不影响结果），而覆盖的连续整数数目要求正好等于最大整数的两倍加1，也就是所有的两数和都要落在指定范围。  
