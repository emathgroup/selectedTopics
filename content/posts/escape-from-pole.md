...
mathe根据变分法，给出了一个路线在极坐标下的微分方程$\theta u^2u^{\prime\prime}=-(1+u^{\prime})^3+2\theta u(1+u^{\prime})^2-(u+\theta)u(1+u^{\prime})+\theta u^3$，不过最初mathe弄错了一个符号，所以大家一直未能利用这个微分方程来求得合理得解。
