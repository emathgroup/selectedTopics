...
KeyTo9\_Fans最早[给出了一个精度不错得数值解](https://bbs.emath.ac.cn/forum.php?mod=redirect&goto=findpost&ptid=307&pid=41144&fromuid=20) : 3.549260。  
mathe尝试使用变分法，给出了一个路线在极坐标下的微分方程$\theta u^2u^{\prime\prime}=-(1+u^{\prime})^3+2\theta u(1+u^{\prime})^2-(u+\theta)u(1+u^{\prime})+\theta u^3$，不过最初mathe微分方程算错了，弄错了一个符号，所以大家一直未能利用这个微分方程来求得合理得解。  
...
为了解决这个期望路径最短问题，[据说KeyTo9先给出了一种先直行再拐弯绕行](https://bbs.emath.ac.cn/forum.php?mod=redirect&goto=findpost&ptid=307&pid=2729&fromuid=20) 方案的示意图  
...
mathe说他重复采用这种方法在将角度和u都分成1000份左右时，可以得出类似精度的结果，但是发现即使再细分，精度也无法提升了。
