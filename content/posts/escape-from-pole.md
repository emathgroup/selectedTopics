...
$\int_0^{2\pi-2d} \theta \frac{ds}{d\theta} d\theta = \int_0^{2\pi-2d} \theta \sqrt(u^2+(1+u'^2) ) d\theta$,  
也就是我们要求上面积分的最小值。  
...
注意最后一次对$\theta$求导时$u,u^{\prime}$都要看成$\theta$的函数,而$u'$就是$\frac{du}{d\theta}$  
...
然后由于$u'-vu+1=0$,假设方程有两个不同的解$u_1,u_2$,得出$(u^{\prime}_1-u^{\prime}_2)=v(u_1-v_2)$所以$u_1-u_2=C\exp(\int_0^{\theta}v(x)dx)$  
