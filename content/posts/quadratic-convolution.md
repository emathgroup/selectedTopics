...
二次对合变换  
定义1: 如果对于一个可逆的$3\time 3$对称阵C,点$(x_1,y_1 )$和点$(x_2,y_2 )$之间满足关系式  
$(x_1^2,x_1 y_1,y_1^2 )C\begin{pmatrix}x_2^2\\x_2 y_\\@y_2^2\end{pmatrix}=0$,我们称这是点$(x_1,y_1 )$到点$(x_2,y_2 )$的二次对合变换。  
...
现在我们查看椭圆曲线$y^2=u_0 (x^3+u_2 x^2+u_1 x+u_0)$,显然上面有点 $(0,u_0)$,现在我们假设曲线上还有一点$(h,∓\sqrt{-4u_0 g(-h) })$,我们连接这两点得到直线$y=u_0+\frac{∓\sqrt{-4u_0 g(-h)}-u_0}{h} x$,然后代入椭圆方程得到$u_0 (x^3+u_2 x^2+u_1 x+u_0)-(u_0+\frac{∓\sqrt{-4u_0 g(-h) }-u_0}{h} x)^2=0$,由于我们知道这个方程已经有解0，h，得到第三个解为$\frac{u_1±2 \frac{\sqrt{-4u_0 g(-h) }-u_0}h}{h}=\frac{u_1 h+2u_0±\sqrt{-4u_0 g(-h)}}{h^2} =f$，由此得出这个二阶对合变换群同椭圆曲线$y^2=u_0 (x^3+u_2 x^2+u_1 x+u_0)$的群是完全同构的，而且曲线的横坐标对应二次曲线系中的参数h，纵坐标的符号决定了变换的方向。
...
```
得到这个椭圆曲线群,而且前面的单位点对应X=0的情况就是$\[0,\frac{64}9\]$  
于是我们可以用命令
```bash
gp > a=[0,64/9];
gp > b=elladd(E,a,a)
